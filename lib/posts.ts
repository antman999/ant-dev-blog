...
export async function getExperience() {
  const res = await fetch(
    "https://raw.githubusercontent.com/antman999/portfolio-data/main/experience.mdx",
    {
      headers: {
...
export async function getPostByName(
  ...
  const res = await fetch(
    `https://raw.githubusercontent.com/antman999/portfolio-data/main/${filePath}`,
    {
      headers: {
        Accept: "application/vnd.github+json",
        Authorization: `Bearer ${process.env.GITHUB_TOKEN}`,
...
export async function getBlogPostMeta(): Promise<MetaBlog[] | undefined> {
  const res = await fetch(
    "https://api.github.com/repos/antman999/portfolio-data/git/trees/main?recursive=1",
    {
      headers: {
        Accept: "application/vnd.github+json",
        Authorization: `Bearer ${process.env.GITHUB_TOKEN}`,
  ...
  const blogpostsContents = (repoFileTree.tree as any[])
    .map((item) => {
      if (
        item.path.startsWith("blogposts/") &&
        item.type === "blob" &&
